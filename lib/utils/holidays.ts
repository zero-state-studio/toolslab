export interface HolidayDecoration {
  emoji: string;
  name: string;
  /** Optional Tailwind ring class to highlight the icon border */
  ringClass?: string;
}
...
export function getCurrentHoliday(now?: Date): HolidayDecoration | null {
  ...
  if (month === 1 && day >= 5 && day <= 6) {
    return { emoji: '⭐', name: "Epiphany" };
  }

  // Valentine's Day (Feb 12–14)
  if (month === 2 && day >= 12 && day <= 14) {
    return { emoji: '💝', name: "Valentine's Day" };
  }

  // St. Patrick's Day (Mar 15–17)
  if (month === 3 && day >= 15 && day <= 17) {
    return { emoji: '🍀', name: "St. Patrick's Day" };
  }

  // Easter (Palm Sunday –2 days before to Easter Monday)
  ...
  // April Fools (Apr 1)
  if (month === 4 && day === 1) {
    return { emoji: '🃏', name: "April Fools" };
  }

  // International Workers' Day (May 1)
  if (month === 5 && day === 1) {
    return { emoji: '✊', name: "Workers' Day" };
  }

  // Halloween (Oct 28–31)
  if (month === 10 && day >= 28 && day <= 31) {
    return { emoji: '🎃', name: "Halloween" };
  }

  // Day of the Dead (Nov 1–2)
  if (month === 11 && day >= 1 && day <= 2) {
    return { emoji: '💀', name: "Day of the Dead" };
  }
